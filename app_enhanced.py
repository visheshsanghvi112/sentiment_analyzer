...
try:
    ...
    st.error(f"❌ Error importing new modules: {e}")
    st.stop()
...
def display_movie_analysis(movie_name: str):
    ...
    if st.button("🤖 Get AI Analysis", key="ai_analysis"):
        with st.spinner("Analyzing with AI..."):
            try:
                from gemini_utils import get_emotional_insights_for_movie
                emotional_insights = get_emotional_insights_for_movie(movie_name, movie_reviews)
                st.subheader("🧠 AI Emotional Analysis")
                st.write(emotional_insights)
            except Exception as e:
                                ...
                                with st.expander(f"#{i+1}: {movie} (similarity: {similarity:.2f})"):
                                    ...
                                    try:
                                        explanation = explain_movie_recommendation(
                                            selected_movie, movie, source_features, target_features, similarity
                                        )
                                        st.write(explanation)
                                    except Exception as e:
            ...
            if recommendations:
                ...
                if st.button("🤖 Get AI Analysis"):
                    with st.spinner("Analyzing recommendations..."):
                        try:
                            analysis = analyze_mood_based_search(mood_description, recommendations)
                            st.subheader("🧠 AI Analysis")
                            st.write(analysis)
                        except Exception as e:
                            st.error(f"Error getting AI analysis: {e}")
